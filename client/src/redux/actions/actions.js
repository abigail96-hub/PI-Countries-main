...
return async function (dispatch){
    try {
      let allCountries = await axios.get("/country")
      dispatch({
        type: 'GET_COUNTRIES',
...
export function getCountryByName(name){
  ...
  return async function(dispatch){
    try{
        let countryByName = await axios.get(`/country?name=${name}`)
        return dispatch({
...
export function postActivity(payload){
  return async function (dispatch){
      try{
          let activity = await axios.post(`/activity/`, payload)
          return dispatch({
              type: 'POST_ACTIVITY',
...
export function getActivities(){
  ...
  return async function (dispatch) {
      try{
          let allActivities = await axios.get("/activity/");
